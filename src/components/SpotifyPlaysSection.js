...
export default class SpotifyPlays extends React.Component {
  ...
  async componentDidMount() {
    ...
    const uniqueRecentlyPlayed = recentlyPlayed.filter((obj, pos, arr) => {
      return (
        arr.map(mapObj => mapObj.track.name).indexOf(obj.track.name) === pos
      );
    });

    this.setState({
      topPlayed,
      recentlyPlayed: uniqueRecentlyPlayed,
      isLoading: false
    });
  }

  render() {
    const {topPlayed, recentlyPlayed, isLoading} = this.state;
    return (
      <Section>
        <SectionTitle
                ...
                return (
                  <div
                    ...
                    <SpotifyRecentlyPlayedMedia
                      ...
                      url={object.track.album.external_urls.spotify}
                    />
                  </div>
                );
              })}
